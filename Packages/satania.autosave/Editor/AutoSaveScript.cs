...
namespace satania.runtime.autosave
{
	...
	public class AutoSaveScript : EditorWindow
    {
		...
		private void ShowGUI()
		{
			...
			{
				nextTime = EditorApplication.timeSinceStartup + timeArray[intervalTime];
				Debug.Log($"{timeArray[intervalTime] / 60}分起きに設定しました。");
			}
		}
        ...
        {
			if (isChangedHierarchy && EditorApplication.timeSinceStartup > nextTime)
			{
				nextTime = EditorApplication.timeSinceStartup + timeArray[intervalTime];
